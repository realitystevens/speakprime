...
function highlightFiller(text: string) {
  ...
  return parts.map((part, i) =>
    FILLER_WORDS.some((fw) => fw.toLowerCase() === part.toLowerCase()) ? (
      <mark key={i} className="bg-amber-500/25 text-amber-500 rounded-[2px] px-0.5">{part}</mark>
    ) : (part)
  );
}
...
function LiveInterviewInner() {
  ...
  const [confidence] = useState<"strong" | "moderate" | "low">("moderate");
  ...
  const setupMic = useCallback(async (ctx: AudioContext) => {
    try {
      ...
      worklet.port.onmessage = (e: MessageEvent) => {
        if (wsRef.current?.readyState === WebSocket.OPEN && !mutedRef.current && !pausedRef.current) {
          wsRef.current.send((e.data.pcm as Int16Array).buffer as ArrayBuffer);
        }
      };
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
          ...
          case "transcript":
            if (msg.speaker === "ai") {
              ...
              setTranscriptEntries((prev) => {
                ...
                if (last?.speaker === "ai") {
                  return [...prev.slice(0, -1), { speaker: "ai", text: last.text + (msg.text ?? "") }];
                }
                return [...prev, { speaker: "ai" as const, text: msg.text ?? "" }];
              });
            } else {
              const text: string = msg.text ?? "";
              setFillerCount((c) => c + countFillers(text));
              setTranscriptEntries((prev) => {
                const last = prev[prev.length - 1];
                if (last?.speaker === "user") {
                  return [...prev.slice(0, -1), { speaker: "user", text: last.text + " " + text }];
                }
                return [...prev, { speaker: "user" as const, text }];
              });
            }
            break;
          case "feedback": {
            ...
            setFeedbacks((prev) => [...prev.slice(-3), { id, type, text: msg.message ?? "" }]);
            setTimeout(() => setFeedbacks((prev) => prev.filter((f) => f.id !== id)), 8000);
            break;
          }
          case "interrupted":
            setAiSpeaking(false);
      ...
      } catch { /* non-JSON binary handled above */ }
    };
  ...
  const sendMuteState = useCallback((isMuted: boolean) => {
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      wsRef.current.send(JSON.stringify({ type: isMuted ? "mute" : "unmute" }));
    }
  }, []);

  const handleMuteToggle = () => {
    setMuted((m) => { sendMuteState(!m); return !m; });
  };

  const handlePauseToggle = () => {
    setPaused((p) => { sendMuteState(!p); return !p; });
  };
  ...
  return (
    <div className="h-screen w-screen overflow-hidden relative select-none bg-[#1c1c1c]">
      ...
      <div className={`absolute inset-0 flex items-center justify-center transition-[padding] duration-300 ${showPanel ? "sm:pr-[380px]" : ""}`}>
        ...
        <div
          className={`relative flex flex-col items-center justify-center rounded-3xl w-[min(580px,88vw)] px-12 py-[52px] bg-[rgba(40,40,40,0.85)] backdrop-blur-xl transition-[border,box-shadow] duration-[400ms] ${aiSpeaking ? "border border-blue-500/40 shadow-[0_0_60px_rgba(59,130,246,0.15)]" : "border border-white/[0.07] shadow-[0_24px_80px_rgba(0,0,0,0.7)]"
            }`}
        >
          <div className="relative mb-6">
            <div className="w-28 h-28 rounded-full flex items-center justify-center bg-gradient-to-br from-blue-700 to-violet-700">
          ...
          <p className="text-[#F8FAFC] text-xl font-bold mb-1.5">Speakprime AI Interviewer</p>
          <div className="flex items-center gap-2 mb-8">
            {aiSpeaking ? (
              ...
              <span className="text-slate-500 text-[13px]">Connecting…</span>
            ) : wsStatus === "error" ? (
              <span className="text-red-400 text-[13px]">Connection error — check backend</span>
            ) : (
              <>
          ...
          <div className="w-full rounded-2xl p-5 bg-blue-500/[0.08] border border-blue-500/20">
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.08em] mb-2">Current Question</p>
            <p className="text-[#F8FAFC] text-[15px] leading-[1.65] font-medium">
              {currentQuestion || "Waiting for the AI interviewer to begin…"}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* ── USER WEBCAM PIP ── */}
      <div className={`absolute z-20 rounded-2xl overflow-hidden bottom-[90px] w-[200px] h-[148px] border-2 border-white/[0.12] shadow-[0_8px_32px_rgba(0,0,0,0.7)] bg-[#111827] transition-[right] duration-300 ease-out right-4 ${showPanel ? "sm:right-[396px]" : ""}`}>
      ...
      <div className="absolute top-4 left-4 right-4 flex items-start justify-between z-20 gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full backdrop-blur-sm" style={{ background: confidenceConfig.bg, border: `1px solid ${confidenceConfig.color}35` }}>
          ...
          <div className="hidden sm:flex px-3 py-1.5 rounded-full bg-black/55 backdrop-blur-xl border border-white/10">
      ...
      <div className="absolute z-20 space-y-2 bottom-[90px] left-4 max-w-[340px]">
        {feedbacks.map((f) => (
          <div key={f.id} className="feedback-toast px-4 py-3 rounded-2xl bg-[rgba(18,18,18,0.92)] backdrop-blur-2xl shadow-[0_4px_20px_rgba(0,0,0,0.5)]"
            style={{ borderLeft: `3px solid ${feedbackColors[f.type].border}`, border: `1px solid ${feedbackColors[f.type].border}25`, borderLeftWidth: "3px" }}>
            <span className="text-slate-100 text-[13px] leading-[1.5]">{emoji[f.type]} {f.text}</span>
          </div>
        ))}
      </div>
      ...
      <div className={`absolute bottom-5 left-0 right-0 flex items-center justify-center z-20 transition-[padding] duration-300 ${showPanel ? "sm:pr-[380px]" : ""}`}>
        <div className="flex items-center gap-3 px-5 py-3 rounded-full bg-[rgba(10,14,26,0.88)] backdrop-blur-2xl border border-white/[0.09] shadow-[0_8px_40px_rgba(0,0,0,0.7)]">
          <div className="hidden md:flex items-center gap-2 pr-4 border-r border-white/10">
            <div className="w-28 h-1 rounded-full overflow-hidden bg-white/10">
              <div className="h-full rounded-full transition-all duration-1000" style={{ width: `${Math.min((timer / (30 * 60)) * 100, 100)}%`, background: "linear-gradient(90deg, #3B82F6, #8B5CF6)" }} />
            </div>
      ...
      {/* ── RIGHT COACH PANEL ── */}
      {showPanel && (
        <div className="coach-panel fixed inset-0 sm:absolute sm:inset-auto sm:top-0 sm:right-0 sm:bottom-0 z-30 flex flex-col w-full sm:w-[380px] bg-[rgba(22,22,22,0.97)] backdrop-blur-[32px] border-l border-white/[0.07]">
          ...
          <div className="px-5 py-4 border-b border-white/[0.07]">
            ...
            <div className="p-4 rounded-xl bg-blue-500/[0.08] border border-blue-500/[0.18]">
              <p className="text-[#F8FAFC] text-[13px] leading-[1.65] font-medium">
                {currentQuestion || "Waiting for the AI to begin…"}
              </p>
            </div>
          </div>
          <div className="flex-1 overflow-auto px-5 py-4">
            ...
            <div className="space-y-3">
              {transcriptEntries.length === 0 ? (
                <p className="text-[#334155] text-xs">Transcript will appear here once the session starts…</p>
              ) : (
                transcriptEntries.map((entry, i) => (
                  <div key={i}>
                    <p className="text-slate-600 text-[10px] font-bold uppercase tracking-[0.05em] mb-0.5">
                      {entry.speaker === "ai" ? "AI Interviewer" : "You"}
                    </p>
                    <p className="text-slate-400 text-[13px] leading-[1.75]">
                      {entry.speaker === "user" ? highlightFiller(entry.text) : entry.text}
                    </p>
                  </div>
                ))
              )}
              {timer > 0 && <span className="inline-block w-1.5 h-4 rounded-sm ml-1 bg-blue-500 animate-waveform" />}
            </div>
          </div>
          <div className="px-5 py-4 border-t border-white/[0.07]">
            ...
            <div className="space-y-2 max-h-[200px] overflow-hidden">
              {feedbacks.length === 0 ? (
                ...
                <div key={f.id} className="feedback-toast p-3 rounded-xl"
                  style={{ background: feedbackColors[f.type].bg, borderLeft: `3px solid ${feedbackColors[f.type].border}` }}>
                  <span className="text-slate-200 text-xs leading-[1.5]">{emoji[f.type]} {f.text}</span>
                </div>
              ))}
            </div>
          </div>
